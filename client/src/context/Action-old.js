...
//   let uploaded;
//   const uploadTask = storage.ref(`/videos/posts/${video.filename}`).put(video.file);
//     uploadTask.on(
//       "state_changed",
//       (snapshot) => {
//         const progress =
...
export const uploadPictures = async(images, setUrls) =>{
  try {
    let arr = [];
    images.forEach(async(image)=>{
      const ref = storage.ref(`/images/posts/${image.filename}`);
...
const uploadImages = (images)=>{
  ...
  images.forEach((image) => {
    const uploadTask = storage.ref(`/images/posts/${image.filename}`).put(image.file);
    uploadTask.on(
      "state_changed",
      (snapshot) => {
        const progress =
  ...
  try {
    console.log(post);
      ...
      if (data.images.length > 0) {
        ...
        // });
        // console.log(temp);
        // if(temp.length === data.images.length){
        //   post.images = temp;
        //   console.log(post);
        // }
      }
